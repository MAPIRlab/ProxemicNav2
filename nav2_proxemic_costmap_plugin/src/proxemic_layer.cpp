...
void ProxemicLayer::updateBounds(double robot_x, double robot_y, double robot_yaw, double * min_x, double * min_y, double * max_x, double * max_y){ 
    ...
    //if(!enabled_){return;}
    auto node = node_.lock();
    RCLCPP_INFO(node->get_logger(),"updatebounds");
...
void ProxemicLayer::updateCosts(nav2_costmap_2d::Costmap2D & master_grid, int min_i, int min_j, int max_i, int max_j)
{
    ...
    auto node = node_.lock();
    RCLCPP_INFO(node->get_logger(),"updatecosts");
    ...
    if(update_cost_){
        ...
        // __________________________________________________CON ESTAS LÍNEAS ME LANZA EL ERROR PARA COORDENADAS RANDOM________________________
        // int map_x = 0;
        // int map_y = 0;
        ...
        // ___________________________________CON ESTAS LÍNEAS ME LANZA EL ERROR PARA CUALQUIER COORDENADA NEGATIVA, Y PARA ALGUNAS COMO (1,3)________________________
        unsigned int map_x = 0;
        unsigned int map_y = 0;                                                       
        bool bien = worldToMap(posex, posey, map_x, map_y);
        if(!bien){
        RCLCPP_INFO(node->get_logger(),"ERROR EN WORLD TO MAP");
        }
        ...
        for (int j = min_j; j < max_j; j++) {
            for (int i = min_i; i < max_i; i++) {
                ...
                setCost(i, j, cost);
                //proxemic_costmap_->setCost(i, j, cost);
            }
        }
        ...
        RCLCPP_INFO(node->get_logger(),"exito");
    }
